...
Операция возведения в степень, выполняется справа на лево. Например:

2<sup>3<sup>4</sup></sup> будет записана как `2 ** 3 ** 4` и выполняться будет в следующей последовательности: 
сначало: **3<sup>4</sup> = 81**, а потом **2<sup>81</sup> = 2417851639229258349412352**
...
```
+----+---------------------------+---------------------------------------------------+---------------------------------+
| is | Отношение тождественности | Показывает как левый операнд отождествляется      | True is True ==> True,          |
|    |                           | правому операнду. Истинно тогда и только          | 5 is 4 ==> False                |
|    |                           | тогда, когда оба операнда являются одним и тем же |                                 |
|    |                           | объектом.                                         |                                 |
+----+---------------------------+---------------------------------------------------+---------------------------------+
| in | Отношение принадлежности  | Показывает принадлежит ли левый оперант некоторому| 5 in [1, 2, 3, 4, 5] ==> True   |
|    |                           |  мнодеству представленному правым операндом.      | 10 in (1, 2, 3, 4, 5) ==> False |
+----+---------------------------+---------------------------------------------------+---------------------------------+
```
